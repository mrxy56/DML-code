...
if __name__ == "__main__":
  ...
  # Training flags.
  flags.DEFINE_integer("num_gpu", 7,
...
def get_input_data_tensors(reader,
  ...
  with tf.name_scope("train_input"):
    ...
    #train_shape = training_data[0][1].get_shape().as_list()
    #mapped_training_data = []
    #with tf.variable_scope("foo") as foo_scope:
    #    fusing = tf.get_variable('fusing', shape = [train_shape[2], 1152],
    #                             initializer = tf.truncated_normal_initializer(stddev=0.01),
    #                                trainable=True)
    #for data in training_data:
    #    train_shape = data[1].get_shape().as_list()
    #    with tf.variable_scope(foo_scope, reuse=True):
    #        fusing = tf.get_variable('fusing')
    #    reshaped_data = tf.reshape(data[1], [train_shape[0] * train_shape[1], train_shape[2]])
    #    fused_data = tf.matmul(reshaped_data, fusing)
    #    final_data = tf.reshape(fused_data, [train_shape[0], train_shape[1], 1152])
    #    new_data = (data[0], final_data, data[2], data[3])
    #    mapped_training_data.append(new_data)
    
    #with tf.variable_scope("foo") as foo_scope:
    #   fusing = tf.get_variable('fusing', shape = [1024, 576],
    #                             initializer = tf.truncated_normal_initializer(stddev=0.01),
    #                                trainable=True)
    #   fusing1 = tf.get_variable('fusing1', shape = [128, 576],
    #                             initializer = tf.truncated_normal_initializer(stddev=0.01),
    #                                trainable=True)
    #for data in training_data:
    #   train_shape = data[1].get_shape().as_list()
    #   with tf.variable_scope(foo_scope, reuse=True):
    #       fusing = tf.get_variable('fusing')
    #       fusing1 = tf.get_variable('fusing1')
    #   reshaped_data = tf.reshape(data[1][:,:,:1024], [train_shape[0] * train_shape[1], 1024])
    #   fused_data = tf.matmul(reshaped_data, fusing)
    #   reshaped_data1 = tf.reshape(data[1][:,:,1024:],[train_shape[0] * train_shape[1], 128])
    #   fused_data1 = tf.matmul(reshaped_data1, fusing1)
    #   final_fused_data = tf.concat([fused_data, fused_data1], axis=1)
    #   final_data = tf.reshape(final_fused_data, [train_shape[0], train_shape[1], 1152])
    #   new_data = (data[0], final_data, data[2], data[3])
    #   mapped_training_data.append(new_data)
    ...
    with sv.managed_session(target, config=self.config) as sess:
      try:
        ...
        while (not sv.should_stop()) and (not self.max_steps_reached):
          ...
          _, global_step_val, loss_val, predictions_val, labels_val, model_input_val = sess.run(
              [train_op, global_step, loss, predictions, labels, model_input])
          logging.info("printing...")
          logging.info(model_input_val.shape)
